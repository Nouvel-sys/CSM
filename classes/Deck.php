...
final class Deck {
    ...
    public function owned(string $id,string $userId): ?array { return $this->db->query('SELECT * FROM decks WHERE id=? AND user_id=?',[$id,$userId])->fetch() ?: null; }
    public function allForUser(string $userId): array { return $this->db->query('SELECT * FROM decks WHERE user_id=? ORDER BY updated_at DESC',[$userId])->fetchAll(); }
