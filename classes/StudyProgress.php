...
final class StudyProgress {
    ...
    public function record(string $user,string $card,string $result): void {
        ...
        $this->db->query("INSERT INTO bombcard_progress (user_id,bombcard_id,times_seen,times_known,times_unknown,status,last_reviewed_at)
            SELECT ?,c.id,1,?,?,IF(?=1,'known','learning'),CURRENT_TIMESTAMP(3) FROM cards c JOIN decks d ON d.id=c.deck_id WHERE c.id=? AND d.user_id=? AND d.moderation_status='visible'
    ...
    public function forUser(string $user): array {
        return $this->db->query("SELECT p.*,c.deck_id FROM bombcard_progress p JOIN cards c ON c.id=p.bombcard_id JOIN decks d ON d.id=c.deck_id WHERE p.user_id=? AND d.user_id=? AND d.moderation_status='visible' ORDER BY p.last_reviewed_at DESC",[$user,$user])->fetchAll();
    }
}
