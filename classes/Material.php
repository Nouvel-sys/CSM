...
final class Material {
    ...
    public function owned(string $id,string $userId): ?array { return $this->db->query("SELECT m.* FROM documents m JOIN decks d ON d.id=m.deck_id WHERE m.id=? AND m.user_id=? AND d.moderation_status='visible'",[$id,$userId])->fetch() ?: null; }
