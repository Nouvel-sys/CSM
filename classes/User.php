...
final class User {
    ...
    public function setPassword(string $id,string $hash): void { $this->db->query('UPDATE users SET password_hash=?,auth_version=auth_version+1 WHERE id=?',[$hash,$id]); }
    public function profile(string $id): array {
        $row=$this->db->query('SELECT u.username,u.email,p.* FROM users u JOIN profiles p ON p.user_id=u.id WHERE u.id=?',[$id])->fetch();
        if(!$row)fail(404,'Account not found.');return ['username'=>$row['username'],'email'=>$row['email'],'displayName'=>$row['display_name'],'avatar'=>$row['avatar'],'nameChangedAt'=>timestamp_ms($row['name_changed_at'])];
    }
}
